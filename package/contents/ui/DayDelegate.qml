...
MouseArea {
	...
	Text {
		...
		font.pixelSize: {
			if (today && todayStyle == "bigNumber") {
				...
				return Math.max(Kirigami.Theme.smallFont.pixelSize, Math.floor(base * 0.9))
			}
		}
	...
	PlasmaCore.ToolTipArea {
		...
		mainText: containsMouse ? thisDate.toLocaleDateString(Qt.locale(), Qt.locale().dateFormat(Locale.LongFormat)) : ""
