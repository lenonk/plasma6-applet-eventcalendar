...
if [ ! -f "$metadataFile" ]; then
	echoRed "[build] Error: Missing $metadataFile"
	exit 1
fi
...
packageVersion=`metadataValue "Version"`
packageWebsite=`metadataValue "Website"`
packageComment=`metadataValue "Description"`

if [ -z "$packageNamespace" ] || [ -z "$packageVersion" ]; then
	echoRed "[build] Error: Could not read Id/Version from package/metadata.json"
	exit 1
fi
